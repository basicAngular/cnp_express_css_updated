    ...
    <div class="page-header clearfix">
        <div class="pull-right">
            @if($user_data->hasAccess(['broker.write']) || $user_data->inRole('admin'))
                <a href="{{ $type.'/add-admin' }}" class="btn btn-primary">
                    <i class="fa fa-plus-circle"></i> Create Admin </a>
            @endif
            @if($user_data->inRole('admin'))
                <a href="{{ $type.'/invite' }}" class="btn btn-warning">
                    <i class="fa fa-envelope"></i> Invite Admin</a>
            @endif
        </div>
    </div>
            ...
            $(document).ready(function () {
                oTable = $('#data').DataTable({
                    ...
                    "ajax": "{{ url($type) }}" + ((typeof $('#data').attr('data-id') != "undefined") ? "/" + $('#id')
                            .val() + "/" + $('#data').attr('data-id') : "/adminData")
                });
            });
        </script>
    @endif
